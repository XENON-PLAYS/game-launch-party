...
export function GameRequestList() {
  ...
  const handleUpdateStatus = async (id: string, status: 'fulfilled' | 'denied') => {
    try {
      ...
      toast.success(`Pedido ${status === 'fulfilled' ? 'atendido' : 'negado'} com sucesso!`);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "fulfilled":
        return <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 gap-1"><CheckCircle2 className="h-3 w-3" /> Atendido</Badge>;
      case "denied":
        return <Badge className="bg-destructive/10 text-destructive border-destructive/20 gap-1"><XCircle className="h-3 w-3" /> Negado</Badge>;
      default:
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card/50 p-6 rounded-3xl border border-border/40 backdrop-blur-xl">
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight">Pedidos de Jogos</h2>
      ...
      <div className="grid grid-cols-1 gap-4">
              ...
              {request.status === 'pending' && (
                <div className="flex items-center gap-2 self-end md:self-center">
                  ...
                  <Button 
                    size="sm"
                    variant="outline"
                    className="h-10 rounded-xl border-destructive/20 hover:bg-destructive/10 hover:text-destructive font-bold uppercase tracking-widest text-[10px]"
        ...
        {filteredRequests.length === 0 && !isLoading && (
          <div className="py-20 text-center space-y-4">
            ...
            <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest opacity-60">Nenhum pedido encontrado</p>
          </div>
        )}
      </div>
    </div>
